export const WORKFLOW_STATES = [
  ...
  'VALIDATION_FAILED','RESEARCH_FAILED','QC_FAILED','INTEGRATION_FAILED','PUBLISH_FAILED','NEEDS_ATTENTION'
] as const;
...
export interface ScoreBreakdown {
  pain: number; financialConsequence: number; researchBurden: number; demandEvidence: number;
  existingSpending: number; informationFragmentation: number; pdfSuitability: number; reachability: number;
}
...
export function validateScore(s: ScoreBreakdown): void {
  (Object.keys(SCORE_MAX) as (keyof ScoreBreakdown)[]).forEach(k => { if (s[k] < 0 || s[k] > SCORE_MAX[k]) throw new Error(`Invalid score for ${k}`); });
}

const transitions: Record<string, WorkflowState[]> = {
  OPPORTUNITY_DISCOVERED:['VALIDATING'], VALIDATING:['VALIDATED','VALIDATION_FAILED'], VALIDATED:['RESEARCHING'], RESEARCHING:['RESEARCH_COMPLETE','RESEARCH_FAILED'], RESEARCH_COMPLETE:['COMPETITOR_ANALYSIS'], COMPETITOR_ANALYSIS:['PRODUCT_PROPOSED'], PRODUCT_PROPOSED:['AWAITING_APPROVAL'], AWAITING_APPROVAL:['APPROVED','OPPORTUNITY_DISCOVERED'], APPROVED:['BUILDING'], BUILDING:['QC'], QC:['QC_PASSED','QC_FAILED'], QC_FAILED:['BUILDING'], QC_PASSED:['DESIGNING'], DESIGNING:['READY_TO_PUBLISH','INTEGRATION_FAILED'], READY_TO_PUBLISH:['AWAITING_PUBLISH_APPROVAL'], AWAITING_PUBLISH_APPROVAL:['PUBLISHED','READY_TO_PUBLISH'], PUBLISHED:['MARKETING'], MARKETING:['ANALYTICS'], ANALYTICS:['IMPROVEMENT'], IMPROVEMENT:['OPPORTUNITY_DISCOVERED'], INTEGRATION_FAILED:['DESIGNING'], PUBLISH_FAILED:['AWAITING_PUBLISH_APPROVAL'], RESEARCH_FAILED:['RESEARCHING'], VALIDATION_FAILED:['OPPORTUNITY_DISCOVERED'], NEEDS_ATTENTION:['NEEDS_ATTENTION']
};
